  ...
  try {
    ...
    try {
      ...
      if (!bucketExists) {
        ...
        return NextResponse.json({ 
          error: 'Storage bucket not found. Please create "course-images" bucket in Supabase Dashboard.',
          details: {
      ...
      return NextResponse.json({ 
        error: 'Unable to access storage. Please check Supabase configuration.',
    ...
    try {
      ...
      if (uploadError) {
        ...
        // Method 2: Alternative upload path
        const altFileName = `courses/${fileName}`
